# Program na początku sortuje pierwszy przedział [0 ... p - 1] w O(p logp)
...
# jako że kopiec min_heap może rozrosnąć się w najgorszym przypadku do n, a operacje na nim wykonywane są w czasie
# logarytmicznym, to dostajemy log(n) operacji na kopce. pętla for() wykonuje się (n - p) razy, w najgorszym wypadku n, zatem
# ostateczna złożoność algorytmu wynosi O(n*log(n))
...
def ksum(T, k, p):
    ...
    for i in range(n - p + 1):
        sum += calc_kth(T, max_heap, min_heap, is_in_min_heap, n, i, p)

    return sum

def calc_kth(T:list, max_heap:list, min_heap:list, is_in_min_heap:list, n:int, i:int, p:int):
...
def heap_insert(heap:list, val:int, ind:int, heap_flag:bool):
    heap.append((val, ind))
    pos = len(heap) - 1
    if heap_flag:
...
def heap_pop(heap:list, heap_flag:bool):
    heap[0], heap[-1] = heap[-1], heap[0]
    heap.pop()
    heapify(heap, len(heap), 0, heap_flag)

def build_heap(T, heap_flag):
    n = len(T)
    for i in range(parent(n - 1), -1, -1):
        heapify(T, n, i, heap_flag)

def heapify(T, n, i, heap_flag):
    l, r = left(i), right(i)
    mi = i
    if heap_flag:
